...
interface FavouriteDao {
    ...
    suspend fun upsert(data: Model.response_favourite)

    @Query("select * from table_favourites")
    fun getAllDataLive(): LiveData<List<Model.response_favourite>>

    @Query("select * from table_favourites")
    suspend fun getAllData(): List<Model.response_favourite>
    ...
    @Query("select exists(select 1 from table_favourites where movieId = :movieId limit 1)")
    suspend fun isDataExist(movieId: Int): Boolean
}