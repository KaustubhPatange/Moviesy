...
class DownloadService : IntentService("blank") {
    ...
    override fun onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val notificationChannel = NotificationChannel(getString(R.string.CHANNEL_ID_1), "ytsdownload", NotificationManager.IMPORTANCE_LOW)
            notificationChannel.description = "Movie Download Service"
            val notificationManager: NotificationManager? = context.getSystemService(NotificationManager::class.java)
            notificationManager?.createNotificationChannel(notificationChannel)

            val channel = NotificationChannel(getString(R.string.CHANNEL_ID_2), "Download", NotificationManager.IMPORTANCE_LOW)
        ...
        torrentStream.addListener(object : TorrentListener{
            ...
            override fun onStreamProgress(torrent: com.github.se_bastiaan.torrentstream.Torrent?, status: StreamStatus?) {
                if (lastProgress==0f || lastProgress != status?.progress) {
                    ...
                    DS_LOG("==> Progress: ${status?.progress}, Download queue: ${torrent?.torrentHandle?.downloadQueue?.size}, " +
    ...
    fun updateNotification(model: Torrent, torrent: com.github.se_bastiaan.torrentstream.Torrent?
        ...
        if (status!=null) {
            ...
            torrentJob = TorrentJob(
                model.title,model.banner_url,status.progress.toInt(),status.seeds,status.downloadSpeed,
                0,torrent?.torrentHandle?.torrentFile()?.totalSize(),true,"Downloading",
                torrent?.torrentHandle?.peerInfo()?.size as Int,magnetHash
            )
        }else {
        ...
        setContentIntent(model)

        val speedString = formatDownloadSpeed(speed as Float)

        val notificationBuilder = NotificationCompat.Builder(context, getString(R.string.CHANNEL_ID_1))
